...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.bookingDetail.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.booking-details.update', [$bookingDetail->id]) }}"
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('booking_no') ? 'is-invalid' : '' }}" type="text"
                    name="booking_no" id="booking_no" value="{{ old('booking_no', $bookingDetail->booking_no) }}"
                    required>
            ...
            <div class="form-group">
                ...
                <input class="form-control datetime {{ $errors->has('booking_date_time') ? 'is-invalid' : '' }}"
                    type="text" name="booking_date_time" id="booking_date_time"
                    value="{{ old('booking_date_time', $bookingDetail->booking_date_time) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service') ? 'is-invalid' : '' }}" name="service_id"
                    ...
                    <option value="{{ $id }}" {{ (old('service_id') ? old('service_id') : $bookingDetail->service->id ??
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_provider') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ (old('service_provider_id') ? old('service_provider_id') :
                        $bookingDetail->service_provider->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('client') ? 'is-invalid' : '' }}" name="client_id"
                    ...
                    <option value="{{ $id }}" {{ (old('client_id') ? old('client_id') : $bookingDetail->client->id ??
                        '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" type="text" name="status"
                    id="status" value="{{ old('status', $bookingDetail->status) }}" required>
