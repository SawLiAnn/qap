        ...
        <form method="POST" action="{{ route('admin.payment-transactions.store') }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('invoice_no') ? 'is-invalid' : '' }}" type="text"
                    name="invoice_no" id="invoice_no" value="{{ old('invoice_no', '') }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('booking_details') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ old('booking_details_id')==$id ? 'selected' : '' }}>{{ $entry }}
                    </option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('paid_with') ? 'is-invalid' : '' }}" type="text"
                    name="paid_with" id="paid_with" value="{{ old('paid_with', '') }}" required>
