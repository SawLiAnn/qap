...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.serviceProviderSchedule.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route('admin.service-provider-schedules.update', [$serviceProviderSchedule->id])
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('day_of_week') ? 'is-invalid' : '' }}" type="text"
                    ...
                    value="{{ old('day_of_week', $serviceProviderSchedule->day_of_week) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('start_time') ? 'is-invalid' : '' }}" type="text"
                    ...
                    value="{{ old('start_time', $serviceProviderSchedule->start_time) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control timepicker {{ $errors->has('end_time') ? 'is-invalid' : '' }}" type="text"
                    name="end_time" id="end_time" value="{{ old('end_time', $serviceProviderSchedule->end_time) }}"
                    required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('service_provider') ? 'is-invalid' : '' }}"
                    ...
                    <option value="{{ $id }}" {{ (old('service_provider_id') ? old('service_provider_id') :
                        $serviceProviderSchedule->service_provider->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}
                    </option>
                    @endforeach
                </select>
